...
2. If the line that has the highest percent_het_consistent is also the line with the highest percent_hom_consistent, and the percent_hom_consistent is over 0.9, then that is the best match. Else, the best match is 'undetermined'.
...
BestMatches <- dat %>%
  ...
  ungroup() %>%
  filter(BestHit_hom & BestHit_het & perc_hom_consistent >= 0.9) %>%
  right_join(
    dat %>% dplyr::select(ExpectedSampleID) %>% unique(),
    by = "ExpectedSampleID"
  ) %>%
  dplyr::select(ExpectedSampleID, BestMatch = SampleID, fn) %>%
