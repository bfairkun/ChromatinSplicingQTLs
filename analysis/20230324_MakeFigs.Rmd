...
#### Short detour
Yang wants a beta beta scatter plot for hQTLs vs eQTLs in different datasets

```{r}

dat.recoded$PC2 %>% unique()
dat.recoded$PC1 %>% unique()
...
# ggsave("/project2/yangili1/carlos_and_ben_shared/rough_figs/OriginalSubplots/20230412_WIP_eQTLhQTL_H3K4ME3_betascatter.pdf", height=2.5, width=10)

```



### Back to thinking about classifying txn vs post-txn eQTLs...
...
### beta beta scatter but for hQTL/eQTLs
...
#Units for expression are log2CPM
#Units for splicing is leafcutter PSI
...
MergedEstimatesFromModel.deltas %>%
  filter(!is.na(SuperAnnotation)) %>%
  mutate(IsProductive = if_else(str_detect(SuperAnnotation, "Unproductive"), "Unproductive", "Productive")) %>%
  ggplot(aes(x=Estimate.Splicing.FC, y=Estimate.Expression.FC)) +
  geom_point(alpha=0.2) +
  facet_grid(IsProductive~dose.nM) +
  theme_bw() +
  labs(x="Splicing change\nDeltaPSI of GAGT intron", y="Expression change\nlog2FC of host gene")

MergedEstimatesFromModel.deltas %>%
  filter(!is.na(SuperAnnotation)) %>%
  mutate(IsProductive = if_else(str_detect(SuperAnnotation, "Unannotated"), "Unannotated", "Annotated")) %>%
  filter(dose.nM == 3160) %>%
  mutate(sign= sign(Estimate.Expression.FC)) %>%
  filter(IsProductive=="Unannotated") %>%
  ggplot(aes(x=Estimate.Splicing.FC, y=Estimate.Expression.FC, color=factor(sign))) +
  geom_point(alpha=1) +
  facet_grid(IsProductive~dose.nM) +
  theme_bw() +
  labs(x="Splicing change\nDeltaPSI of GAGT intron", y="Expression change\nlog2FC of host gene")
...
MergedData.polyA.chRNA.Expression.Splicing <- MergedEstimatesFromModel.deltas %>%
  inner_join(
    chRNA.splicing.dat %>%
      mutate(dose.nM = as.numeric(dose.nM)) %>%
      mutate(Intron = str_replace(intron, "^chr(.+?):clu_.+$", "\\1")),
    by=c("dose.nM", "Intron")
    ) %>%
  inner_join(
    chRNA.DE.dat %>%
      separate(Geneid, into=c("Geneid", "symbol"), sep="_"),
    by=c("symbol", "Geneid", "dose.nM"="risdiplam_conc")
  ) %>%
  mutate(IsProductive = if_else(str_detect(SuperAnnotation, "Unproductive"), "Unproductive", "Productive"))


ggplot(MergedData.polyA.chRNA.Expression.Splicing, aes(x=deltapsi*100, y=Estimate.Splicing.deltaPSI, color=IsProductive)) +
  geom_point() +
  geom_abline() +
  labs(x="chRNA deltaPSI", y="polyA deltaPSI")

ggplot(MergedData.polyA.chRNA.Expression.Splicing, aes(x=logef, y=Estimate.Splicing.FC, color=IsProductive)) +
  geom_point() +
  geom_abline() +
  labs(x="chRNA splicing log2FC", y="polyA splicing log2FC")

ggplot(MergedData.polyA.chRNA.Expression.Splicing, aes(x=logFC, y=Estimate.Expression.FC, color=IsProductive)) +
  geom_point() +
  geom_abline() +
  labs(x="chRNA HostExpression log2FC", y="polyA HostExpression log2FC")

ggplot(MergedData.polyA.chRNA.Expression.Splicing, aes(x=logFC, y=Estimate.Expression.FC, color=SemiSupergroupAnnotations)) +
  geom_point() +
  geom_abline() +
  labs(x="chRNA HostExpression log2FC", y="polyA HostExpression log2FC") +
  facet_wrap(~dose.nM) +
...
But why are so many of the chRNA effects positive?
...
chRNA.DE.dat %>%
  ...
  labs(x="chRNA expression logFC", y="ecdf")

  
```

How many introns per cluster in these 100 genes

```{r}

JuncsOfInterest <- MergedData.polyA.chRNA.Expression.Splicing %>%
  distinct(junc) %>% pull(junc)

leafcutter.table <- read.table("../code/SmallMolecule/leafcutter/clustering/autosomes/leafcutter_perind_numers.counts.gz", sep=' ')

#chr1:2189781:2192647:-

leafcutter.table %>%
  rownames_to_column("rowname") %>%
  dplyr::select(rowname) %>%
  mutate(junc_cluster = str_replace(rowname, "^(.+?:)(clu_.+?)_([+-])$", "\\1\\3;\\2")) %>%
  separate(junc_cluster, into=c("junc", "cluster"), sep=";") %>%
  group_by(cluster) %>%
  filter(any(junc %in% JuncsOfInterest)) %>%
  ungroup() %>%
  count(cluster)
  
```

I think the deltaPSI vs logFC expression plots will look better if I base PSI not on leafcutter PSI, but rather on PSI calculated just from junction reads in junction triplets for cassette exons... 

```{r}
cassette.exons <- read_tsv("../output/SmallMoleculeGAGT_CassetteExonclusters.bed",col_names=c("chrom","start", "end","name", "score", "strand", "thickStart", "thickEnd", "color" )) 

cassette.exons %>%
  filter(str_detect(name, "junc.GAGT"))
```

Based on those cassette exon (intron trios), in the snakemake I recalculated PSI and fit dose response curves, and now I can repeat some of the previous plots...

```{r}
small.molecule.dat.introns.asPSI <- read_tsv("../code/SmallMolecule/FitModels/polyA_GAGTIntrons_asPSI.tsv.gz") %>%
  mutate(Intron = str_replace(junc, "^chr(.+?):[+-]$", "\\1")) %>%
  inner_join(IntronAnnotatins)
...
MergedEstimatesFromModel <- right_join(
  small.molecule.dat.introns.asPSI %>%
    filter(str_detect(param, "Pred")) %>%
    mutate(gene = str_replace(gene, "(^.+?)\\..+?", "\\1")),
  small.molecule.dat.genes %>%
    filter(str_detect(param, "Pred")) %>%
...
MergedEstimatesFromModel.deltas %>%
  filter(!is.na(SuperAnnotation)) %>%
  distinct(gene, dose.nM, .keep_all=T) %>%
  mutate(IsProductive = if_else(str_detect(SuperAnnotation, "Unproductive"), "Unproductive", "Productive")) %>%
  ggplot(aes(x=Estimate.Splicing.FC, y=Estimate.Expression.FC, color=SemiSupergroupAnnotations)) +
  geom_point() +
  facet_grid(IsProductive~dose.nM) +
  theme_bw() +
  labs(x="Splicing change\nDeltaPSI of GAGT intron", y="Expression change\nlog2FC of host gene")
...
Ok, actually that doesn't look so different.. One thing that is kind of annoying is how we only have ~100 introns, when I know I modelled close to 350...


```{r}
 read_tsv("../code/SmallMolecule/FitModels/polyA_GAGTIntrons_asPSI.tsv.gz") %>%
   distinct(junc) %>% nrow()
...
Now that I think of it, the reason why is that I am joining on Yang's annotations (which I'm not sure he included my small molecule unique junctions)... But since intron trios are likely to contain something in Yang's annotation file, I think I can still figure it out. And this one way to link introns to host genes

```{r}
cassette.exons %>%
  mutate(chrom = str_remove_all(chrom, "chr")) %>%
  left_join(IntronAnnotatins) %>% head()

GAGTjuncs_to_genes <- cassette.exons %>%
  mutate(chrom = str_remove_all(chrom, "chr")) %>%
  separate(name, into=c("type", "dummy", "clusterNum", "GAGTjunc"), sep="_", convert=T) %>%
  left_join(IntronAnnotatins) %>%
  distinct(GAGTjunc, gene) %>%
  inner_join(
    cassette.exons %>%
      separate(name, into=c("type", "dummy", "clusterNum", "GAGTjunc"), sep="_", convert=T) %>%
      filter(type %in% c("junc.GAGT", "junc.UpstreamIntron")) %>%
      dplyr::select(GAGTjunc, start, end, type, strand) %>%
      pivot_wider(names_from="type", values_from=c("start", "end")) %>%
      mutate(ExonLen = case_when(
        strand == "+" ~ start_junc.GAGT - end_junc.UpstreamIntron -1,
        strand == "-" ~ start_junc.UpstreamIntron - end_junc.GAGT -1
      )) %>%
      dplyr::select(GAGTjunc, ExonLen)
  )
...
small.molecule.dat.introns.asPSI <- read_tsv("../code/SmallMolecule/FitModels/polyA_GAGTIntrons_asPSI.tsv.gz") %>%
  ...
  left_join(GAGTjuncs_to_genes, by=c("junc"="GAGTjunc")) %>%
  mutate(ExonLenRemainder = (ExonLen-1)%%3)
...
MergedEstimatesFromModel.deltas %>%
  mutate(ExonLenRemainder= as.character(ExonLenRemainder)) %>%
  mutate(IsProductive = case_when(
    is.na(SuperAnnotation) ~ ExonLenRemainder,
    TRUE ~ SuperAnnotation
  )) %>%
  ggplot(aes(x=Estimate.Splicing.deltaPSI, y=Estimate.Expression.FC, color=IsProductive)) +
  geom_point() +
  facet_grid(IsProductive~dose.nM) +
  theme_bw() +
  labs(x="Splicing change\nDeltaPSI of GAGT intron", y="Expression change\nlog2FC of host gene")
...
#     TRUE ~ SuperAnnotation
#   )) %>%
#   filter(Estimate.Splicing.FC) %>%
#   ggplot(aes(x=Estimate.Splicing.FC, y=Estimate.Expression.FC, color=IsProductive)) +
#   geom_point() +
#   facet_grid(IsProductive~dose.nM) +
#   theme_bw() +
#   labs(x="Splicing change\nDeltaPSI of GAGT intron", y="Expression change\nlog2FC of host gene")
...
Ok, this is getting messy, and not clean effects... I think it will actually be worth the time to translate the cassette exon from the annotated frame...

I did that in the snakemake... Briefly, I extracted the flanking exons (using the highest expressed transcript structure from salmon quantifications when their are multiple transcript isoforms that could be used to define the flanking exons), then translated the exons with or without the cassette exon.

```{r}
Translated.GAGT_CassetteExons <- read_tsv("../code/SmallMolecule/CassetteExons/ExonsToTranslate.Translated.tsv.gz") %>%
  dplyr::select(-chrom, -Strand, -gene) %>%
...
MergedEstimatesFromModel.deltas$SuperAnnotation %>% unique()

Translated.GAGT_CassetteExons %>%
  count(ExonRemainder, Effect) %>%
  ggplot(aes(x=ExonRemainder, y=n, fill=Effect)) +
  geom_col(position='stack') +
  labs(x="CassetteExon Length %% 3", y="Count", fill="Effect of exon inclusion")
...
MergedEstimatesFromModel.deltas %>%
  ...
  ggplot(aes(x=Estimate.Splicing.FC, y=Estimate.Expression.FC, color=as.factor(ExonLenRemainder))) +
  geom_hline(yintercept = 0, linetype='dashed') +
  geom_point() +
  facet_grid(IsProductive~dose.nM) +
  # facet_grid(~dose.nM) +
...
MergedEstimatesFromModel.deltas %>%
  ...
  ggplot(aes(x=Estimate.Splicing.deltaPSI, y=Estimate.Expression.FC, color=as.factor(ExonLenRemainder))) +
  geom_hline(yintercept = 0, linetype='dashed') +
  geom_point() +
  facet_grid(IsProductive~dose.nM) +
  # facet_grid(~dose.nM) +
...
MergedEstimatesFromModel.deltas %>%
  ...
  ggplot(aes(x=Estimate.Expression.FC, color=Effect)) +
  geom_vline(xintercept=0, linetype='dashed') +
  stat_ecdf() +
  facet_grid(~dose.nM) +
  theme_bw() +
  labs(x="Expression change\nlog2FC of host gene", y="ecdf")
