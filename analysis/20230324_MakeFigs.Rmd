  ...
  inner_join(
    cassette.exons %>%
      ...
      mutate(ExonLen = case_when(
        strand == "+" ~ start_junc.GAGT - end_junc.UpstreamIntron,
        strand == "-" ~ start_junc.UpstreamIntron - end_junc.GAGT
      )) %>%
...
small.molecule.dat.introns.asPSI <- read_tsv("../code/SmallMolecule/FitModels/polyA_GAGTIntrons_asPSI.tsv.gz") %>%
  ...
  mutate(ExonLenRemainder = ExonLen%%3)
...
Some pseudocode approach to doing that...

get flanking basic exons (and their transcripts, sometimes there will be more than one)... filter for the most used transcript from salmon quantifications... then start translating with and without the cassette exon.

```{r}
IntersectingExons <- read_tsv("../code/SmallMolecule/CassetteExons/FlankingExons.tsv.gz", col_names=c("SkipJuncChrom", "SkipJuncStart", "SkipJuncStop", "SkipJuncName", "SkipJuncScore", "Strand", "dummy_thickStart", "dummy_thickEnd", "dummy_color", "ExonChrom","ExonStart", "ExonStop", "gene_transcript", "dummy", "dummy_2", "overlappingBases")) %>%
  dplyr::select(-contains("dummy")) %>%
  separate(gene_transcript, into=c("gene", "transcript"), sep="\\_")

TPM.transcripts.DMSO <- read_tsv("../code/SmallMolecule/salmon.DMSO.merged.txt") %>%
...
hist(log10(TPM.transcripts.DMSO$Median.TPM))
...
IntersectingExons %>%
  ...
  ungroup() %>%
  add_count(SkipJuncName) %>%
  filter(n==2) %>%
  filter(overlappingBases==1) %>%
  distinct(SkipJuncName) %>%
  nrow()
