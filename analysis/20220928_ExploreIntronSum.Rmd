...
dat.to.plot <- dat.exonic.expression.rpkm %>%
  as.data.frame() %>%
  rownames_to_column("Geneid") %>%
  dplyr::select(Geneid, contains("chRNA"), contains("polyA")) %>%
  pivot_longer(-Geneid, names_sep=";", names_to=c("Sample", "Type")) %>%
  group_by(Type, Geneid) %>%
  summarise(MedianLogRPKM = median(value))  %>%
  ungroup() %>%
  pivot_wider(names_from="Type", values_from="MedianLogRPKM") %>%
  mutate(PLCL1 = str_detect(Geneid, "ENSG00000115896")) %>%
  mutate(alpha= if_else(PLCL1, 1, 0)) %>%
  mutate(label = if_else(PLCL1, "PLCL1", NA_character_)) %>%
  arrange(PLCL1) %>%
  mutate(Diff = chRNA - polyA) %>%
  mutate(rank = dense_rank(Diff) )

ggplot(dat.to.plot, aes(x=chRNA, y=polyA, color=PLCL1)) +
...
```
```{r, eval=F}
pdf(file="../code/scratch/heatmap_intronic_exonic_geneexpression.pdf")
BetweenVsWithinIndividualCorrelations %>%
  group_by(Assay.A, Assay.B, Comparison) %>%
  summarise(medianCorr = median(CorrelationCoef)) %>%
  ungroup() %>%
  ...
  column_to_rownames("Assay.A") %>%
  as.matrix() %>%
  heatmap.2(trace="none", col=viridis(50, direction = -1), cexRow=0.3, cexCol=0.3)
dev.off()
```
