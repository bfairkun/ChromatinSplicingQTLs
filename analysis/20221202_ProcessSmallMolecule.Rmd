...
left_join(
  ...
  PSI.ByDonor.Summary %>% filter(dose.nM == 0) %>%
    dplyr::select(Is.GA.GT, PSI.summary),
  by=c("Is.GA.GT")
) %>%
  mutate(MeanPSIFoldChange=(PSI.summary.x-PSI.summary.y)) %>%
  ggplot(aes(x=dose.nM, y=MeanPSIFoldChange, group=Is.GA.GT, color=Is.GA.GT=="GA")) +
  geom_line() +
  geom_point() + 
...
Mean.PSI <- PSI.tidy %>%
  filter(LibraryType == "polyA") %>%
  group_by(Intron, dose.nM, DonorSeq) %>%
...
inner_join(
  ...
  Mean.PSI %>%
    filter(dose.nM==0) %>%
    dplyr::select(Intron, DonorSeq, PSI),
  by=c("Intron", "DonorSeq")
) %>%
  mutate(Is.GA.GT = substr(DonorSeq, 3,4)) %>%
  ...
  ungroup() %>%
  drop_na() %>%
  ggplot(aes(x=dose.nM, y=PSI.summary, group=Is.GA.GT, color=Is.GA.GT=="GA")) +
  geom_line() +
  geom_point() + 
  scale_x_continuous(trans="log1p", limits=c(0, 10000), breaks=c(10000, 3160, 1000, 316, 100, 31.6, 10, 3.16, 0), labels=c(10000, 3160, 1000, 316, 100, 31.6, 10, 3.16, 0)) +
  # facet_wrap(~Is.GA.GT, scales = "free_y") +
