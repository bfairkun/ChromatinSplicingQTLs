...
> one thing I'd still like to see is whether you see the correlation pattern when you start will all top K36me3 QTLs or H3K27ac QTL at promotor

```{r}
P1Category <- c("H3K36ME3", "H3K27AC", "chRNA.Expression.Splicing","MetabolicLabelled.30min","MetabolicLabelled.60min", "Expression.Splicing.Subset_YRI")
P2Category <- "polyA.Splicing"
...
Compare_sQTL_eQTL_effects_across_datasets <- TopSNPEffects.ByPairs %>%
  filter(
      ...
      (FDR.x < 0.01)) %>%
  filter(
    (paste(GeneLocus, P1, sep=';') %in% PeaksToTSS$GenePeakPair) | (!PC1 == "H3K27AC")
  ) %>%
  ...
  # ungroup() %>%
  unite(TraitPair, P1, PC1, P2, PC2, remove=F) %>%
  dplyr::select(TraitPair, beta1=beta.x, beta2=trait.x.beta.in.y, P1, PC1, P2, PC2, IntronAnnotation, sQTL.P = trait.x.p.in.y) %>%
  mutate(Is.Concordant = sign(beta1)==sign(beta2)) %>%
  mutate(ConsistentWithNMD = case_when(
    !IntronAnnotation == "Annotated basic" & Is.Concordant ~ "Inconsistent with NMD",
    !IntronAnnotation == "Annotated basic" & !Is.Concordant ~ "Consistent with NMD",
  ...
  distinct(PC1, P1, IntronAnnotation, .keep_all=T) %>%
  group_by(P1) %>%
  mutate(Is.sQTL = 
           any(PC2 == "polyA.Splicing" & sQTL.P < 0.01)) %>%
  ungroup() %>%
  filter(!is.na(Is.sQTL)) %>%
  mutate(PC1 = recode(PC1, !!!c("H3K27AC"="H3K27AC@TSS","chRNA.Expression.Splicing"="chRNA","MetabolicLabelled.30min"="30min 4sU","MetabolicLabelled.60min"="60min 4sU", "Expression.Splicing.Subset_YRI"="polyA RNA"))) %>%
  mutate(PC1 = factor(PC1, levels=c("H3K27AC@TSS","H3K36ME3","chRNA", "30min 4sU", "60min 4sU", "polyA RNA")))

Compare_sQTL_eQTL_effects_across_datasets.CorStats <- Compare_sQTL_eQTL_effects_across_datasets %>%
  group_by(IntronAnnotation, PC1, Is.sQTL) %>%
  summarise(cor=cor.test(beta1,beta2)[["estimate"]], pval=cor.test(beta1,beta2, method='s')[["p.value"]], n=n()) %>%
  mutate(R = signif(cor, 3), P=format.pval(pval, 3)) %>%
  mutate(label = str_glue("rho:{R};P:{P};n:{n}")) %>%
  group_by(IntronAnnotation, PC1) %>%
  mutate(rn = row_number())

P.sQTL.effects.Across.eQTL.datasets <- Compare_sQTL_eQTL_effects_across_datasets %>%
  ggplot(aes(x=beta1, y=beta2, color=Is.sQTL)) +
  geom_point(alpha=0.1) +
  geom_vline(xintercept=0) +
  geom_hline(yintercept=0) +
  geom_text(
    data = Compare_sQTL_eQTL_effects_across_datasets.CorStats,
    aes(x=-Inf, y=-Inf, label=label, color=Is.sQTL, vjust=-rn),
    hjust=-.1, size=2
  ) +
  geom_abline(
    data = Compare_sQTL_eQTL_effects_across_datasets.CorStats,
    aes(slope=cor, intercept=0, color=Is.sQTL)) +
  facet_grid(PC1 ~ IntronAnnotation) +
  theme_classic() +
  coord_fixed() +
  labs(title="genetic effects of NMD introns and host gene expression", x="eQTL beta\n(standardized units)", y="sQTL beta\n(standardized units)", caption=str_wrap("sQTL betas relative to eQTL beta at top eQTL SNP. sQTLs defined by nominal P<0.01", 30), color=str_wrap("Is sQTL")) +
  guides(colour = guide_legend(override.aes = list(alpha = 1)))
P.sQTL.effects.Across.eQTL.datasets

PositionsOfFeatures <- paste0("../code/QTLs/QTLTools/", c(P1Category, P2Category), "/OnlyFirstReps.sorted.qqnorm.bed.gz") %>%
  setNames(c(P1Category, P2Category)) %>%
  lapply(fread, select=1:6, sep='\t') %>%
  bind_rows(.id="PC") %>%
  unite(PCP, PC, pid, sep=';')
```

```{r, eval=F}
ggsave("/project2/yangili1/carlos_and_ben_shared/eQTL.to.sQTL.Effects.AllRNA.png",P.sQTL.effects.Across.eQTL.datasets, height=8, width=8)

Max <- 0.25

ColMap <- col_numeric(palette = cividis(18), domain=c(-Max,Max))

show_col(palette = cividis(18))
...
TopSNPEffects.ByPairs %>%
  ...
  ungroup() %>%
  filter(!is.na(Is.sQTL)) %>%
  filter(IntronAnnotation == 'Unannotated') %>%
  mutate(score = case_when(
    beta1*beta2 > Max ~ Max,
    beta1*beta2 < -Max ~ -Max,
    TRUE ~ beta1*beta2)) %>%
  pull(score) %>% hist()
  mutate(RgbCol = apply(col2rgb(ColMap(score)), 2, paste, collapse=',')) %>%
  dplyr::select(TraitPair, score, RgbCol, P1, PC1, P2, PC2) %>%
  unite(PCP1, PC1, P1, sep=';') %>%
  unite(PCP2, PC2, P2, sep=';') %>%
  gather("Trait1_Or_Trait2", "PCP", PCP1, PCP2) %>%
  inner_join(PositionsOfFeatures) %>%
  mutate(thickStart = start, thickEnd=end) %>%
  unite(name, gid, TraitPair) %>%
  dplyr::select(`#Chr`, start, end, name, score, strand, thickStart, thickEnd, RgbCol) %>%
  arrange(`#Chr`, start, end) %>%
  write_tsv("../code/scratch/CheckSlopeFeats.bed")


#Write out bed files for browsing positions of things
Compare_sQTL_eQTL_effects_across_datasets %>%
...
Yang wants to plot boxpltos of unproductive, Annotated NMD, and predicted+annotated NMD
...
ggsave("/project2/yangili1/carlos_and_ben_shared/rough_figs/OriginalSubplots/20230505_PercentNonBasicTag.pdf", width=2.5, height=3)
...
dat %>%
  left_join(OldAnnotationToNewAnnotationKey) %>%
  filter(!str_detect(SuperAnnotation, "Noncoding")) %>%
  mutate(IsFocusCategory = str_detect(SemiSupergroupAnnotations, "nonsense") | str_detect(SemiSupergroupAnnotations, "NMD")) %>%
  group_by(IsFocusCategory, Dataset, IndID) %>%
  summarise(sum = sum(Count)) %>%
  ungroup() %>%
  group_by(Dataset, IndID) %>%
  ...
  ylim(c(0,2)) +
  Rotate_x_labels +
  labs(title="predicted + annotated NMD", y="Percent splice juncs")
ggsave("/project2/yangili1/carlos_and_ben_shared/rough_figs/OriginalSubplots/20230505_PercentTotalPredictedNMD.pdf", width=2.5, height=3)

dat %>%
  left_join(OldAnnotationToNewAnnotationKey) %>%
  filter(!str_detect(SuperAnnotation, "Noncoding")) %>%
  mutate(IsFocusCategory = SemiSupergroupAnnotations == "uniquely nonsense_mediated_decay tag") %>%
  group_by(IsFocusCategory, Dataset, IndID) %>%
  summarise(sum = sum(Count)) %>%
  ungroup() %>%
  group_by(Dataset, IndID) %>%
  ...
  ylim(c(0,1)) +
  Rotate_x_labels +
  labs(title="annotated NMD", y="Percent splice juncs")
ggsave("/project2/yangili1/carlos_and_ben_shared/rough_figs/OriginalSubplots/20230505_PercentAnnotatedNMD.pdf", width=2.5, height=3)


dat %>%
  left_join(OldAnnotationToNewAnnotationKey) %>%
  filter(!str_detect(SuperAnnotation, "Noncoding")) %>%
  mutate(IsFocusCategory = str_detect(SemiSupergroupAnnotations, "nonsense") | str_detect(SemiSupergroupAnnotations, "NMD") | (SemiSupergroupAnnotations == "uniquely processed_transcript tag")) %>%
  group_by(IsFocusCategory, Dataset, IndID) %>%
  summarise(sum = sum(Count)) %>%
  ungroup() %>%
  group_by(Dataset, IndID) %>%
  ...
  ylim(c(0,3.5)) +
  Rotate_x_labels +
  labs(title="annotated NMD + predicted NMD + processed", y="Percent splice juncs")
ggsave("/project2/yangili1/carlos_and_ben_shared/rough_figs/OriginalSubplots/20230505_PercentNMD_andProcessed.pdf", width=2.5, height=3)

```

One last plot for percent in each non basic category
...
dat %>%
  ...
  group_by(Dataset, SemiSupergroupAnnotations) %>%
  summarise(sum=sum(Count)) %>%
  ungroup() %>%
  mutate(Dataset = recode(Dataset, "Expression.Splicing"="polyA", "MetabolicLabelled.30min"="30min", "MetabolicLabelled.60min"="60min", "chRNA.Expression.Splicing"="chRNA")) %>%
  mutate(Dataset = factor(Dataset, levels=c("chRNA", "30min", "60min", "polyA"))) %>%
  ggplot(aes(x=Dataset, y=sum, fill=SemiSupergroupAnnotations)) +
  geom_col(position="fill") +
  labs(y="Percent of non-basic juncs")
...
dat %>%
  ...
  mutate(SemiSupergroupAnnotations = case_when(
    SemiSupergroupAnnotations == "uniquely nonsense_mediated_decay tag" ~ "NMD annotated",
    str_detect(SemiSupergroupAnnotations, "nonsense") | str_detect(SemiSupergroupAnnotations, "NMD") ~ "NMD predicted",
    TRUE ~ SemiSupergroupAnnotations
    )) %>%
  group_by(Dataset, SemiSupergroupAnnotations) %>%
  summarise(sum=sum(Count)) %>%
  ungroup() %>%
...
Yang also requests a scatter of junc RPM in chRNA vs polyA just for certain annotation groups...

```{r}
dat.perjunc <- read_tsv("../code/ReadLengthMapExperimentResults/JuncCountsTidy/SummarisedByJuncAndDataset.txt.gz")

dat.toplot <- dat.perjunc %>%
  filter(Dataset %in% c("chRNA.Expression.Splicing", "Expression.Splicing")) %>%
  unite(Int, chrom, start, end, strand) %>%
  group_by(Dataset, Int) %>%
  summarise(sumPerJunc = sum(Count)) %>%
  ungroup() %>%
  group_by(Dataset) %>%
  mutate(sumForDataset = sum(sumPerJunc)) %>%
  ungroup() %>%
  mutate(RPMJunc = sumPerJunc/sumForDataset*1E6) %>%
  left_join(
    IntronAnnotations %>%
      unite(Int, chrom, start, end, strand),
    by="Int"
  ) %>%
  mutate(AnnotationForPlot = case_when(
    SemiSupergroupAnnotations == "basic tag" ~ "protein coding",
    SemiSupergroupAnnotations == "uniquely processed_transcript tag" ~ "'processed_transcript' tag",
    SemiSupergroupAnnotations == "uniquely nonsense_mediated_decay tag" ~ "NMD annotated",
    str_detect(SemiSupergroupAnnotations, "nonsense") | str_detect(SemiSupergroupAnnotations, "NMD") ~ "NMD predicted",
    TRUE ~ "Other"
  )) %>%
  distinct(Int, Dataset, .keep_all=T) %>%
  dplyr::select(Int, RPMJunc, Dataset, AnnotationForPlot) %>%
  pivot_wider(names_from="Dataset", values_from="RPMJunc") %>%
  mutate(AnnotationForPlot = factor(AnnotationForPlot, levels=c("protein coding", "'processed_transcript' tag", "NMD annotated", "NMD predicted", "Other")))

dat.toplot.labels <- dat.toplot %>%
  mutate(`chRNA.Expression.Splicing`=`chRNA.Expression.Splicing`+1E-3,
         `Expression.Splicing`=`Expression.Splicing`+1E-3) %>%
  mutate(Ratio = `chRNA.Expression.Splicing`/`Expression.Splicing`) %>%
  group_by(AnnotationForPlot) %>%
    summarise(Med = signif(median(Ratio, na.rm=T), 3),
              Num = n()) %>%
  mutate(label = str_glue("n={Num}\nMedian(FC)={Med}")) %>%
  filter(!AnnotationForPlot=="Other")


dat.toplot %>%
  filter(!AnnotationForPlot=="Other") %>%
ggplot(aes(y=Expression.Splicing, x=chRNA.Expression.Splicing)) +
  geom_point(alpha=0.05) +
  geom_abline(color='red') +
  scale_x_continuous(trans='log10') +
  scale_y_continuous(trans='log10') +
  geom_density_2d() +
  geom_text(data =dat.toplot.labels,
            aes(label=label),x=-Inf, y=Inf, color='blue', vjust=1.1, hjust=-.05) +
  facet_wrap(~AnnotationForPlot) +
  labs(x="RPM across juncs, chRNA", y="RPM across juncs, polyA")

ggsave("/project2/yangili1/carlos_and_ben_shared/rough_figs/OriginalSubplots/20230505_JuncScatter.png", width=5.5, height=5)  
```

Yang also requests to see the percent of these non-basic categories in chRNA for each quartile of host gene expression.
...
# OldAnnotationToNewAnnotationKey$SemiSupergroupAnnotations %>% unique()

dat.tidier <- Long.table %>%
  filter(Dataset %in% c("chRNA.Expression.Splicing")) %>%
  left_join(OldAnnotationToNewAnnotationKey) %>%
  filter(!str_detect(SuperAnnotation, "Noncoding")) %>%
  mutate(IsUnrpoductive = str_detect(SuperAnnotation, "Unproductive")) %>%
  group_by(IndID, gene, IsUnrpoductive) %>%
  summarise(SumByProductivity = sum(score)) %>%
  mutate(gene = str_replace(gene, "^(.+?)\\..+?$", "\\1")) %>%
  inner_join(
...
dat.tidier %>%
  ungroup() %>%
  mutate(TPM_quartile = cut_number(medianTPM, n=4)) %>%
  group_by(TPM_quartile, IndID, IsUnrpoductive) %>%
  summarise(SumByProductivityByQuartile = sum(SumByProductivity)) %>%
  ungroup() %>%
  group_by(IndID, TPM_quartile) %>%
  mutate(PercentUnrpdocutiveByQuartile = SumByProductivityByQuartile/sum(SumByProductivityByQuartile)*100) %>%
  ungroup() %>%
  filter(IsUnrpoductive) %>%
  ggplot(aes(x=TPM_quartile, y=PercentUnrpdocutiveByQuartile)) +
  geom_boxplot() +
  ylim(c(0,12)) +
  geom_jitter(width=0.25) +
  Rotate_x_labels +
  labs( y="Percent unproductive\nsplice juncs", x="")
ggsave("/project2/yangili1/carlos_and_ben_shared/rough_figs/OriginalSubplots/20230508_UnproductiveSplicingByHostGeneExpression.pdf", height=3.5, width=3)
...
dat.tidier %>%
  ...
  labs( y="Percent unproductive\nsplice juncs", x="Increasing expression\nquartiles-->")
ggsave("/project2/yangili1/carlos_and_ben_shared/rough_figs/OriginalSubplots/20230508_UnproductiveSplicingByHostGeneExpression2.pdf", height=3, width=3.2)
