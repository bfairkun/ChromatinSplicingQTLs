...
P.PercentAnnotated <- juncs.long.summary %>%
  ...
  ungroup() %>%
  distinct() %>%
  mutate(Dataset = factor(Dataset, levels=c("Expression.Splicing", "MetabolicLabelled.60min","MetabolicLabelled.30min","chRNA.Expression.Splicing"))) %>%
...
P.PercentAnnotated.PerSample <- juncs.long.summary.PerInd %>%
  ungroup() %>%
  filter(!str_detect(SuperAnnotation, "Noncoding")) %>%
...
Let's read in those results and plot nicely...
