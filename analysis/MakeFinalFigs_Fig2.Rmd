...
hQTL.eQTLs.ForCarlos <- inner_join(
  hQTL.eQTL.colocs %>%
    dplyr::select(snp, Locus, beta, P, PC, TopSNPFinemapPr) %>%
    filter(PC=="Expression.Splicing"),
  hQTL.eQTL.colocs %>%
    dplyr::select(snp, Locus, beta, P, PC, TSS_start) %>%
    filter(PC %in% c("H3K4ME3", "H3K27AC", "H3K4ME1")),
  ...
  mutate(BensClassification = case_when(
    ...
    TRUE ~ "hQTL_Distal")) %>%
  dplyr::select(TopSNP=snp, TopSNPFinemapPr, eGene=Locus, eQTL_beta=beta_Expression.Splicing, molQTL_beta=beta_chromatin, molQTL_Name=P_chromatin, molQTL_type=PC_chromatin, EffectSizeDirectionsAsExpected, Dist_from_TopSNP_to_closest_hQTL_TSS,  sQTL_or_hQTL, BensClassification)

sQTL.eQTL.colocs.ForCarlos <- sQTL.eQTL.colocs %>%
  filter(!Locus %in% hQTL.eQTL.colocs$Locus) %>%
  dplyr::select(snp, TopSNPFinemapPr, Locus, beta, P, PC, snp, Locus) %>%
  pivot_wider(names_from="PC", values_from=c("beta", "P")) %>%
  ...
  dplyr::select(TopSNP=snp, TopSNPFinemapPr, eGene=Locus, eQTL_beta=beta_Expression.Splicing, molQTL_beta=beta_polyA.Splicing, molQTL_Name=P_polyA.Splicing, molQTL_type, EffectSizeDirectionsAsExpected, Dist_from_TopSNP_to_closest_hQTL_TSS,  sQTL_or_hQTL, BensClassification)
