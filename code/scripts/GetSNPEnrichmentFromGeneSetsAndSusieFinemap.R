...
if(interactive()){
    args <- scan(text=
                 "QTL_SNP_Enrichment/ContrastingGeneSets/MolColocNonRedundantFullSplicing.tsv.gz QTL_SNP_Enrichment/FinemapIntersections_Susie/Geuvadis.bed.gz scratch/test.Enrichment.", what='character')
} else{
...
# dat.coloc <- read_tsv(FinemapIn, col_names=c("chrom", "start", "stop")) %>%
...
dat.bed <- fread(FinemapIn, col.names=c("SNP_chrom", "SNP_start", "SNP_end", "SNP", "PosteriorPr", "Annotation_chrom", "Annotation_start", "Annotation_stop", "Annotation", "Overlap")) %>%
  separate(SNP, into=c("SNP", "gene", "SignalNum"), sep="_", convert=T) %>%
  filter(SignalNum == "L1") %>%
  inner_join(
    GeneSets %>%
