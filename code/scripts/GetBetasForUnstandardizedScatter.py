...
if hasattr(sys, 'ps1'):
    sys.argv = ["", "../output/TraitPairsForUnnormalizedScatter.ForReviewer.tsv.gz" ,"../output/TraitPairsBetasForUnnormalizedScatter.ForReviewer.tsv.gz", "QTLs/QTLTools/MetabolicLabelled.30min/NominalPassForColocUnstandardized.txt.tabix.gz", "QTLs/QTLTools/MetabolicLabelled.60min/NominalPassForColocUnstandardized.txt.tabix.gz", "QTLs/QTLTools/chRNA.Expression.Splicing/NominalPassForColocUnstandardized.txt.tabix.gz", "QTLs/QTLTools/Expression.Splicing/NominalPassForColocUnstandardized.txt.tabix.gz", "QTLs/QTLTools/polyA.Splicing/NominalPassForColocUnstandardized.txt.tabix.gz"]
...
df = pd.read_csv(f_in, sep='\t', low_memory=False)

TabixFilesDict = {re.search("QTLs/QTLTools/(.+?)/NominalPassForColocUnstandardized.txt.tabix.gz", fn).group(1):pysam.TabixFile(fn, parser=pysam.asTuple()) for fn in tabix_f_in_list}
