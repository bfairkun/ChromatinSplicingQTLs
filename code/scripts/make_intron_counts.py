...
if __name__ == '__main__':
    ...
    ix_list = []
    for sample in tqdm(samples, leave=True, position = 0):
        intron_df = pd.read_csv('SplicingAnalysis/SPLICEq/{phenotype}/{sample}.spliceq.tab'.format(phenotype=phenotype, 
                                                                                                   sample=sample), 
                                sep='\t')
        intron_df['length'] = intron_df.IEnd - intron_df.IStart

        ix = intron_df.groupby(['chr', 'IStart', 'IEnd', 'gene_ID'])[['length', 'intron_cov']].first()
    ...
    IntronCounts = IntronCounts[['Chr', 'start', 'end', 'Geneid', 'strand', 'length'] + samples]
