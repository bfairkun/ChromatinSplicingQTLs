...
rule trim_ncRNAs:
    ...
    shell:
        """
        python scripts/trim_ncRNAs.py --chrom {wildcards.chrom} --strand {wildcards.strand} &> {log}
        """
...
rule FilterAndMergeNcRNAs:
    ...
    shell:
        """
        python scripts/filter_and_merge_ncRNAs.py --chrom {wildcards.chrom} --strand {wildcards.strand} &> {log}
        """
...
rule incRNA_annotated: 
    input:
        lnc = "NonCodingRNA/annotation/tmp/lncRNA.bed.gz",
        protein_coding = "NonCodingRNA/annotation/tmp/protein_coding.bed.gz"
    output:
...
rule GetLncRNAs:
    ...
    shell:
        """
        (zcat {input.genes_bed} | grep 'pseudogene\\|lncRNA' - | gzip - > {output.lnc}) &> {log};
...
rule srtRNA:
    input:
        ncRNA = "NonCodingRNA/annotation/ncRNA.bed.gz",
        lncRNA_bed = "NonCodingRNA/annotation/tmp/lncRNA.bed.gz",
        genes_bed = "NonCodingRNA/annotation/tmp/allGenes.Gencode.bed.gz"
    resources:
        mem_mb = 24000,
    output:
...
rule ClassifyNcRNAs:
    ...
    shell:
        """
        python scripts/make_ncRNA_annotation.py &> {log}
        """
